    ...
    <div className="max-w-xl">
      ...
      <div className="mt-4 rounded-xl border border-line bg-white/60 p-5">
        ...
        <p className="mt-3 text-[13px] text-ink/50">
          EnvoyÃ©e le {new Date(request.createdAt).toLocaleString("fr-FR")}
        </p>
      </div>

      {isBeingValidated && (
        <p className="mt-4 rounded-lg bg-gold-soft px-4 py-3 text-[14px] text-ink">
          ðŸ“ž Un opÃ©rateur KHEDMATI va vous appeler pour confirmer votre demande avant de la
          transmettre aux artisans. C'est notre rÃ¨gle de confiance â€” aucune demande n'est publiÃ©e
          sans validation.
        </p>
      )}
      ...
      {["PUBLISHED", "MATCHING", "PROFESSIONAL_CONTACTED"].includes(request.status) && (
        <p className="mt-4 rounded-lg bg-emerald-soft px-4 py-3 text-[14px] text-emerald-dark">
          âœ… Votre demande est validÃ©e et publiÃ©e. KHEDMATI recherche les professionnels les plus
      ...
      {request.matches?.length > 0 && (
        <div className="mt-6">
          <h2 className="text-[13px] font-medium text-ink/70">Professionnels proposÃ©s</h2>
            ...
            {request.matches.map((m: any) => (
              <div key={m.id} className="rounded-xl border border-line bg-white/60 p-4">
                <p className="text-[15px] text-ink">
                  ...
                  {m.professional?.businessName ? ` â€” ${m.professional.businessName}` : ""}
                </p>
