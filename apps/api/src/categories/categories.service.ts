    ...
    const direct = await this.prisma.profession.findMany({
      where: {
        ...
        OR: [
          ...
          // Recherche aussi dans les sous-catégories (section 4) : "vidange"
          // doit retourner "Mécanicien", même si ce mot n'apparaît jamais
          // dans le nom du métier lui-même.
          { specialties: { some: { name: { contains: normalized, mode: "insensitive" } } } },
          { specialties: { some: { nameAr: { contains: normalized, mode: "insensitive" } } } },
        ],
      },
